...
import AdminDashboardTale from "./dashboardtable";
...
const AdminDashboard = () => {
    ...
    const getStatDashboardProfile = async () => {
      try {
        const res = await axios.get(
          `http://localhost:${PORT}/sous-activite/Admin/State`
        );
  ...
  useEffect(() => {
    const AlldemandesDetails = async () => {
      try {
        const res = await axios.get(
          `http://localhost:${PORT}/personel/AllDemandesWithDetails`
        );
        const data = await res.data;
        setDatatable({
          data: data,
        ...
        <Container maxWidth="lg" className="pt-28">
          <Grid container gap={2}>
            ...
            <Grid
              ...
              md={12}
              lg={6}
              className="py-6 items-start"
            >
              <AdminDashboardTale data={DataTable.data} />
            </Grid>
            <Grid item xs={12} sm={12} md={12} lg={5.5} className="m-auto">
              <AdminChart data={Statdata} />
            </Grid>
          </Grid>
        </Container>  
      </Box>
