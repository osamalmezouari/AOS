...
const UpdateCondoleance: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMaxFiles = async () => {
      try {
        const res = await axios.get(`http://localhost:${PORT}/sous-activite/6`);
    ...
    const fetchDeceasedOptions = async () => {
      try {
        const res = await axios.get(
          `http://localhost:${PORT}/type-condoleance`
        );
  ...
  const handleChange = (
    ...
    setSentStatus((prevStatus) => ({
      ...
      alert: "",
      descriptionLength:
        name === "description" ? value.length : prevStatus.descriptionLength,
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    if (formState.files.length < maxFiles) {
      setSentStatus({
        ...
        descriptionLength: formState.description.length,
      });
      return;
    }
    e.preventDefault();
    try {
      ...
      const response = await axios.patch(
        `http://localhost:${PORT}/demande-condoleance/${demandeId}`,
        formState,
        {
          headers: {
    ...
    <div className="bg-landing h-screen">
      ...
      >
        ...
        >
          ...
          >
            Modification de Demande de Condoléance
          </Typography>
          <Box
            component="form"
            sx={{ mt: 3 }}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <FormControl fullWidth required>
                  <InputLabel id="select-deceased-label">
                    le type de Décédé
                  </InputLabel>
                  <Select
...
export default UpdateCondoleance;
