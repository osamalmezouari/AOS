...
const UpdatePelerinage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMaxFiles = async () => {
      try {
        const res = await axios.get(`http://localhost:${PORT}/sous-activite/2`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://localhost:${PORT}/demande-pelerinage/${demandeId}`,
        formState,
        {
          headers: {
    ...
    <div className="bg-landing h-screen">
      ...
      >
        ...
        >
          ...
          >
            Modification de la Demande de Pèlerinage
          </Typography>
          <Box
            component="form"
            sx={{ mt: 3 }}
...
export default UpdatePelerinage;
